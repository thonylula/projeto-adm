  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingId) {
        ...
        await onAddCompany(newCompanyName, newCompanyCnpj, newCompanyLogo);
      }
        ...
        <form onSubmit={handleSubmit} className="flex flex-col sm:flex-row gap-4 items-end">
          ...
          <div className="flex gap-2 w-full sm:w-auto">
            ...
            <button
              type="submit"
              disabled={isSaving}
              className={`flex-1 sm:flex-none px-6 py-2.5 ${editingId ? 'bg-indigo-600 hover:bg-indigo-700' : 'bg-slate-900 hover:bg-slate-800'} text-white font-medium rounded-lg transition-colors shadow-sm disabled:opacity-50 flex items-center justify-center gap-2`}
            >
              {isSaving && (
              ...
              {editingId ? (isSaving ? 'Salvando...' : 'Salvar Alterações') : (isSaving ? 'Cadastrando...' : 'Cadastrar')}
            </button>
          </div>
        </form>
      </div>
