                            ...
                            {!isPublic && (
                                <td className="px-2 py-2 text-center print:hidden export-ignore">
                                    <div className="flex justify-center gap-1 items-center">
                                        <button type="button" onClick={(e) => onCopySummary(e, generateSmartSummary(item), item.id)} className={`p-1 rounded shadow-sm transition-all duration-300 ${copiedId === item.id ? 'bg-emerald-600' : 'bg-slate-600'}`} title="Copiar Resumo">
                                            {copiedId === item.id ? '✓' : '💬'}
                                        </button>
                                        <button type="button" onClick={() => onIAHolerite(item)} className="p-1 bg-indigo-600 text-white rounded text-[10px] font-bold">IA</button>
                                        <button type="button" onClick={() => onReceipt(item)} className="p-1 bg-orange-600 text-white rounded text-[10px] font-bold">RECIBO</button>
