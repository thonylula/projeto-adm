                                            ...
                                            {(data?.records || []).map((record, index) => (
                                                <React.Fragment key={record.id}>
                                                    <tr className={`transition-all font-bold group transition-colors duration-500 ${isDarkMode
                                                        ...
                                                        <td className={`p-1 border sticky z-10 w-[140px] transition-colors duration-500 shadow-[4px_0_8px_-2px_rgba(0,0,0,0.2)] ${isDarkMode ? 'border-slate-700' : 'border-slate-100'} ${record.status === 'preparacao' ? (isDarkMode ? 'bg-emerald-900/40' : 'bg-[#dcedc8]') : (isDarkMode ? 'bg-[#1E293B]' : 'bg-white')}`} style={{ left: `${tableConfig.veWidth}px` }} rowSpan={2}>
