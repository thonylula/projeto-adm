            ...
            >
                ...
                `}</style>
                        ...
                        <div className={`rounded-xl shadow-sm border m-4 mt-0 max-w-full overflow-hidden transition-colors duration-500 ${isDarkMode ? 'bg-[#1E293B] border-slate-700' : 'bg-white border-slate-100'}`}>
                            ...
                            <div id="interactive-table-container" className="w-full overflow-hidden min-w-0 relative">
                                <div
                                    ref={scrollRef}
                                    ...
                                    <table className="min-w-max w-full border-collapse" style={{
                                        ...
                                        <thead>
                                            <tr className={isDarkMode ? 'bg-slate-900' : 'bg-slate-900'} style={{ height: `${tableConfig.rowHeight * 6}px` }}>
                                                ...
                                                <th className={`p-2 text-white border font-bold uppercase sticky z-20 w-[140px] text-center transition-colors duration-500 ${isDarkMode ? 'border-slate-700 bg-slate-900' : 'border-slate-800 bg-slate-900'}`} style={{ left: `${tableConfig.veWidth}px`, '--sticky-bg': '#0f172a', boxShadow: '1px 0 0 0 var(--sticky-bg), 6px 0 10px -2px rgba(0,0,0,0.3)' } as React.CSSProperties} rowSpan={2}>Data Povoa</th>
                                            ...
                                            {(data?.records || []).map((record, index) => (
                                                <React.Fragment key={record.id}>
                                                    <tr className={`transition-all font-bold group transition-colors duration-500 ${isDarkMode
                                                        ...
                                                        <td className={`p-0 border sticky left-0 z-10 transition-colors duration-500 ${isDarkMode ? 'border-slate-700' : 'border-slate-100'} ${record.status === 'preparacao' ? (isDarkMode ? 'bg-[#064e3b]' : 'bg-[#dcedc8]') : (isDarkMode ? 'bg-[#1E293B]' : 'bg-white')}`} style={{ width: `${tableConfig.veWidth}px` }} rowSpan={2}>
                                                            <div className={`relative h-full flex items-center justify-center font-black border-r transition-colors duration-500 ${isDarkMode ? 'border-slate-700' : 'border-slate-200'} ${record.status === 'preparacao' ? (isDarkMode ? 'bg-[#064e3b]' : 'bg-[#dcedc8]') : (isDarkMode ? 'bg-slate-800/50 text-white' : 'bg-slate-50 text-slate-900')}`} style={{ minHeight: `${tableConfig.lineHeight * 2}px` }}>
                                                                <input
                                                                    type="text"
                                                                    value={record.ve}
                                                        ...
                                                        <td className={`p-1 border sticky z-10 w-[140px] transition-colors duration-500 ${isDarkMode ? 'border-slate-700' : 'border-slate-100'} ${record.status === 'preparacao' ? (isDarkMode ? 'bg-[#064e3b]' : 'bg-[#dcedc8]') : (isDarkMode ? 'bg-[#1E293B]' : 'bg-white')}`} style={{ left: `${tableConfig.veWidth}px`, '--sticky-bg': record.status === 'preparacao' ? (isDarkMode ? '#064e3b' : '#dcedc8') : (isDarkMode ? '#1E293B' : '#ffffff'), boxShadow: '1px 0 0 0 var(--sticky-bg), 6px 0 10px -2px rgba(0,0,0,0.3)' } as React.CSSProperties} rowSpan={2}>
