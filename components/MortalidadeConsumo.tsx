                                    ...
                                    <table className="min-w-max w-full border-collapse" style={{
                                        ...
                                        <thead>
                                            <tr className={isDarkMode ? 'bg-slate-900' : 'bg-slate-900'} style={{ height: `${tableConfig.rowHeight * 6}px` }}>
                                                ...
                                                <th className={`p-2 text-white border font-bold uppercase sticky z-20 w-[140px] text-center transition-colors duration-500 sticky-column-overlay ${isDarkMode ? 'border-slate-700 bg-slate-900' : 'border-slate-800 bg-slate-900'}`} style={{ left: `${tableConfig.veWidth}px`, '--sticky-overlay-bg': '#0f172a' } as React.CSSProperties} rowSpan={2}>Data Povoa</th>
                                            ...
                                            {(data?.records || []).map((record, index) => (
                                                <React.Fragment key={record.id}>
                                                    <tr className={`transition-all font-bold group transition-colors duration-500 ${isDarkMode
                                                        ...
                                                        <td className={`p-1 border sticky z-10 w-[140px] transition-colors duration-500 sticky-column-overlay ${isDarkMode ? 'border-slate-700' : 'border-slate-100'} ${record.status === 'preparacao' ? (isDarkMode ? 'bg-[#064e3b]' : 'bg-[#dcedc8]') : (isDarkMode ? 'bg-[#1E293B]' : 'bg-white')}`} style={{ left: `${tableConfig.veWidth}px`, '--sticky-overlay-bg': record.status === 'preparacao' ? (isDarkMode ? '#064e3b' : '#dcedc8') : (isDarkMode ? '#1E293B' : '#ffffff') } as React.CSSProperties} rowSpan={2}>
